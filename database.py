...
IDENTITY_TABLES = {
    ...
    "emoji_submissions": "id",
    "emoji_submission_reviews": "id",
}
...
CREATE TABLE IF NOT EXISTS emoji_submissions (
    id                INTEGER PRIMARY KEY AUTOINCREMENT,
    original_emoji_id INTEGER NOT NULL UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_emoji_submissions_status
    ON emoji_submissions (status);
CREATE INDEX IF NOT EXISTS idx_emoji_submissions_app_id
    ON emoji_submissions (app_emoji_id);

CREATE TABLE IF NOT EXISTS emoji_submission_reviews (
    id            INTEGER PRIMARY KEY AUTOINCREMENT,
    submission_id INTEGER NOT NULL,
    reviewer_id   INTEGER NOT NULL,
    action        TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS emoji_submissions (
    id                BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    original_emoji_id BIGINT NOT NULL UNIQUE,
    original_name     TEXT NOT NULL,
    animated          INTEGER NOT NULL DEFAULT 0,
    app_emoji_id      BIGINT,
    app_emoji_name    TEXT,
...
class Database:
    ...
    async def ban_guild(self, guild_id: int, banned_by: int, reason: str) -> None:
        await self._execute(
            """
            INSERT INTO banned_guilds (guild_id, banned_by, reason, created_at)
            VALUES (?, ?, ?, ?)
            ON CONFLICT(guild_id) DO UPDATE SET
                banned_by = excluded.banned_by,
                reason = excluded.reason,
                created_at = excluded.created_at
            """,
            (guild_id, banned_by, reason, datetime.utcnow().isoformat()),
        )

    async def unban_guild(self, guild_id: int) -> int:
        return await self._execute("DELETE FROM banned_guilds WHERE guild_id = ?", (guild_id,))
        ...
        return await self._insert_returning_id(
            ...
            INSERT INTO gif_reports
                (url, norm_url, msg_id, channel_id, guild_id, status, created_at,
                 session_type, session_id, sender_user_id, source_guild_id, source_channel_id)
            VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?)
            """,
            (
                url, norm, msg_id, channel_id, guild_id,
                datetime.utcnow().isoformat(), session_type, session_id,
                sender_user_id, source_guild_id, source_channel_id,
            ),
        )
    ...
    async def add_emoji_submission(
        self,
        original_emoji_id: int,
        original_name: str,
        animated: bool,
        submitter_id: int,
        guild_id: int,
        ...
        existing = await self.get_emoji_submission_by_original_id(original_emoji_id)
        if existing:
        ...
        submission_id = await self._insert_returning_id(
            """
            INSERT INTO emoji_submissions
                (original_emoji_id, original_name, animated, submitter_id,
                 guild_id, channel_id, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)
            """,
            (
                original_emoji_id,
    ...
    async def get_emoji_submission(self, submission_id: int) -> Optional[dict]:
        return await self._fetchrow(
            "SELECT * FROM emoji_submissions WHERE id = ?", (submission_id,)
        )

    async def get_emoji_submission_by_original_id(self, original_emoji_id: int) -> Optional[dict]:
        return await self._fetchrow(
            "SELECT * FROM emoji_submissions WHERE original_emoji_id = ?",
            (original_emoji_id,),
        )

    async def set_emoji_submission_review_message(
        self, submission_id: int, message_id: int
    ) -> None:
        await self._execute(
            "UPDATE emoji_submissions SET review_msg_id = ? WHERE id = ?",
            (message_id, submission_id),
        )

    async def review_emoji_submission(
        self,
        submission_id: int,
        reviewer_id: int,
        action: str,
        *,
        ...
        submission = await self.get_emoji_submission(submission_id)
        if not submission or submission["status"] != "pending":
            return None
        updated = await self._execute(
            """
        ...
        await self._insert_returning_id(
            """
            INSERT INTO emoji_submission_reviews
                (submission_id, reviewer_id, action, created_at)
            VALUES (?, ?, ?, ?)
            """,
            (submission_id, reviewer_id, action, datetime.utcnow().isoformat()),
    ...
    async def get_approved_emoji_submissions(
        self, original_emoji_ids: Sequence[int]
    ) -> dict[int, dict]:
        ...
        rows = await self._fetchall(
            ...
            WHERE original_emoji_id IN ({placeholders})
              AND status = 'approved'
    ...
    async def increment_emoji_usage(self, original_emoji_ids: Sequence[int]) -> None:
        ...
        for emoji_id in ids:
            await self._execute(
                ...
                WHERE original_emoji_id = ? AND status = 'approved'
                """,
                (now, emoji_id),
            )

    async def get_emoji_cleanup_candidates(self, cutoff: str, limit: int) -> list[dict]:
        ...
        return await self._fetchall(
            ...
            WHERE status = 'approved'
              AND app_emoji_id IS NOT NULL
              AND (use_count = 0 OR last_used_at IS NULL OR last_used_at < ?)
            ORDER BY use_count ASC, COALESCE(last_used_at, created_at) ASC
            LIMIT ?
            """,
            (cutoff, safe_limit),
        )

    async def mark_emoji_submission_deleted(
        ...
        updated = await self._execute(
            """
            UPDATE emoji_submissions
            SET status = 'deleted', reviewed_at = ?
        ...
        await self._insert_returning_id(
            ...
            VALUES (?, ?, 'deleted', ?)
            """,
            (submission_id, reviewer_id, datetime.utcnow().isoformat()),
        )
        return submission
